...
int main(void){
    ...
    for(int i=0;i<n;i++){
        float k11=h*f1(t[i],u[i],v[i]);
        float k21=h*f2(t[i],u[i],v[i]);
        float k12=h*f1(t[i]+h,u[i]+k11,v[i]+k21);
        float k22=h*f2(t[i]+h,u[i]+k11,v[i]+k21);
        u[i+1]=u[i]+0.5f*(k11+k12);
        v[i+1]=v[i]+0.5f*(k21+k22);
        t[i+1]=t[i]+h;
    }
    printf("\n     t             u             v\n");
